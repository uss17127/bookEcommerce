  ...
  $stmt = $db->prepare("UPDATE orderdetails SET orderType='2', confirmationNumber=$confirmationNumber
  WHERE email='$email' AND orderNumber='$orderNumber'");  
  $stmt->execute();

  $orderTotal = $_SESSION['orderTotal'];

  $stmt = $db->prepare("INSERT INTO ordertable (orderID, email, orderTotal) VALUES ('$orderNumber', '$email', '$orderTotal')");
  $stmt->execute();
  ...
  echo $last;
  echo "!";
  ?></h3> <span class="font-weight-bold d-block mt-4"></span> <span>You reservation has been confirmed!</span>
  ...
  <td>
  <div class="py-2"> <span class="d-block text-muted">Order Id</span> <span><?php echo $orderNumber; ?></span> </div>
  </td>
  <td>
  <div class="py-2"> <span class="d-block text-muted">Confirmation Number</span> <span><?php echo $confirmationNumber; ?></span> </div>
  </td>
  <td>
  <div class="py-2"> <span class="d-block text-muted">Pick Up By</span> <span>
    <script type="text/javascript">
      var myDate = new Date();
  ...
  <?php
      ...
      $stmt = $db->prepare("SELECT * FROM orderdetails WHERE orderNumber=$orderNumber AND confirmationNumber=$confirmationNumber AND orderType='2' ORDER BY ISBN ASC");
      $stmt->execute();
  ...
  ?>
        ...
        $Date = date('Y-m-d', strtotime($Date. ' + 5 days'));
            if ($_SESSION['orderTotal'] != NULL) {  
              $subject = "Red-Wagon Reservation Confirmation";
              $headers = "From: red_wagonbooks@yahoo.com \r\n";
              $headers .= "MIME-Version: 1.0\r\n";
              $headers .= "Content-Type: text/html; charset=ISO-8859-1\r\n";
              $message = "<td><font face='Playfair Display' font size='3'>Hello ".$first."! Thank you for your order with us! Please pick up your books at the store before the reservation expires on: ".$Date."<br>";
            ...
            while ($row = $stmt->fetch()) {
              ...
              $message .= "<span>ISBN: ".$ISBN."</span><br>";
  ...
  <p>We will be sending an reservation confirmation email soon! If the order is not picked up from the bookstore within the alloted time the reservation will be canceled.</p>
