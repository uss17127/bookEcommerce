...
<head>
    ...
    <link rel="stylesheet" href="../searchView/searchView.css">
  ...
  <small>
    ...
    <?php
      if (isset($_POST["submit"])){
        $str = $_POST["search"];
        $sth = $db->prepare("SELECT * FROM book WHERE (Title LIKE '%$str%') OR (Author LIKE '%$str%') OR (ISBN = '$str')");
        $sth -> execute();
      

  if($sth->rowCount() == 0){
    echo "<div><img src='../searchBar/noSearch.jpg' style='width:800px; height:500px'> </div>";
  }
  else{
        while($row = $sth->fetch()){
          $title = $row['Title'];
      ...
      <form name="search" action="../productPage/productPage.php" method="GET">
        ...
        <input type="submit" class="btn btn-outline-dark btn-small" value="View Book" name="submit" />
      </form>
    <?php
       echo "</div>";
        }
